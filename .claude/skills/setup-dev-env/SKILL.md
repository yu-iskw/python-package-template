...
2. **Install CLI toolchain** — From the repo root, run `make setup-tools` (or `mise trust` then `mise install --locked`). This installs trunk, trivy, osv-scanner, grype, and codeql from `mise.toml` / `mise.lock` and runs `trunk install` for Trunk-managed linters.
...
- `mise exec -- trunk --version` succeeds; Python matches `.python-version`; `uv` is available.
